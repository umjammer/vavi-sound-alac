...
class StreamUtils {
    ...
    public static int stream_read(MyStream mystream, int size, byte[] buf, int startPos) {
        ...
        try {
			...
			logger.fine("stream_read: exception thrown: " + err);
        }
    ...
    public static int stream_read_uint32(MyStream mystream) {
        ...
        try {
			...
			logger.fine("stream_read_uint32: exception thrown: " + err);
        }
    ...
    public static void stream_skip(MyStream mystream, int skip) {
        ...
        if (toskip < 0) {
			logger.fine("stream_skip: request to seek backwards in stream - not supported, sorry");
            return;
        }
